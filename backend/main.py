...
async def get_models(
    ...
    with get_db() as conn:
        ...
        for model in models:
            ...
            model_dict['capabilities'] = json.loads(model_dict['capabilities']) if model_dict.get('capabilities') else []
            model_dict['supported_industries'] = json.loads(model_dict['supported_industries']) if model_dict.get('supported_industries') else []
...
async def create_model(model: AIModel, current_user: dict = Depends(get_current_user)):
    if current_user["role"] not in ["organization_admin", "super_admin", "regulatory_inspector"]:
    ...
    with get_db() as conn:
        try:
            ...
            conn.execute(
                ...
                (current_user["id"], "CREATE_MODEL", "ai_model", json.dumps({
                    "model_id": model.id,
...
async def update_model(model_id: str, model_update: dict, current_user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            "INSERT INTO audit_trail (user_id, action, resource_type, details) VALUES (%s, %s, %s, %s)",
            (current_user["id"], "UPDATE_MODEL", "ai_model", json.dumps({
                "model_id": model_id,
...
async def bulk_model_operation(operation: BulkModelOperation, current_user: dict = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            "INSERT INTO audit_trail (user_id, action, resource_type, details) VALUES (%s, %s, %s, %s)",
            (current_user["id"], "BULK_DELETE_MODELS", "ai_model", json.dumps({
        ...
        return {
            "success": True, 
            "data": {"successful": success_count, "failed": failed_count}
        }

@app.get("/api/v1/models/audit")
async def get_model_audit_logs(model_id: Optional[str] = None, limit: int = 50, current_user: dict = Depends(get_current_user)):
    if current_user["role"] not in ["organization_admin", "super_admin", "regulatory_inspector"]:
